...
library FeeMath {
    ...
    function getFeesOwed(
        ...
        uint256 tokenId
    ) internal view returns (BalanceDelta feesOwed) {
        ...
        (uint256 feeGrowthInside0X218, uint256 feeGrowthInside1X128) = manager
            .getFeeGrowthInside(poolId, config.tickLower, config.tickUpper);
    ...
    function getFeesOwed(
        ...
        uint128 token0Owed = getFeeOwed(
            feeGrowthInside0X128,
            feeGrowthInside0LastX128,
            liquidity
        );
        uint128 token1Owed = getFeeOwed(
            feeGrowthInside1X128,
            feeGrowthInside1LastX128,
            liquidity
        );
        feesOwed = toBalanceDelta(
            uint256(token0Owed).toInt128(),
            uint256(token1Owed).toInt128()
        );
    }
    ...
    function getFeeOwed(
        ...
        uint256 liquidity
    ) internal pure returns (uint128 tokenOwed) {
        tokenOwed = (
            FullMath.mulDiv(

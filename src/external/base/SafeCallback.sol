...
abstract contract SafeCallback is ImmutableState, IUnlockCallback {
    ...
    function unlockCallback(bytes memory data) external onlyPoolManager returns (bytes memory) {
        return _unlockCallback(data);
    }

    /// @dev to be implemented by the child contract, to safely guarantee the logic is only executed by the PoolManager
    function _unlockCallback(bytes memory data) internal virtual returns (bytes memory);
}
